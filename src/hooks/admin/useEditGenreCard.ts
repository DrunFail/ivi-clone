...
export default function useEditGenreCard(genreItem: Genre, handleCloseEdit: () => void) {
    const [newGenreName, setNewGenreName] = useState({
        genreNameRu: genreItem.genreNameRu,
        genreNameEng: genreItem.genreNameEng,
    });
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            const response = await MovieAPI.updateGenreById(genreItem.id, {
                genreNameRu: newGenreName.genreNameRu,
                genreNameEng: newGenreName.genreNameEng,
            });
            handleCloseEdit();
        } catch (error) {
            console.log(error);
