    ...
    useEffect(() => {
        if (currentMoviePage.page) {
            fetchMovies(filterParams, currentMoviePage, amountMovieOnPage)
                .then((response) =>
                    setFilteredMovie({ ...filteredMovie, rows: [...filteredMovie.rows, ...response.rows] }),
                )
                .catch((error) => console.log(error));
        }
    }, [amountMovieOnPage, currentMoviePage, filterParams, filteredMovie]);
