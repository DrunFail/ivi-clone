...
import { IClassNames, ICountryObject, IFilterParams, IGenreObject } from "../../../../models";
...
interface FiltersFieldProps {
    ...
    handleChangeFilterWhitoutReload(filterKey: string, filterValue: string): void;
...
const FiltersField = ({
    ...
    const genresRuList = genreObjects.map(genreObject => genreObject.genreNameRu);
    const countriesRuList = countryObjects.map(countryObject => countryObject.countryNameRu);
    const genresEnList = genreObjects.map(genreObject => genreObject.genreNameEng);
    const countriesEnList = countryObjects.map(countryObject => countryObject.countryNameEng);

    /** Данная меняет название страны при смене языка. */
    useEffect(() => {
        if (lang === "En") {
            const countryObject = countryObjects.find(item => item.countryNameRu === filterParams.country);
            if (countryObject) {
                handleChangeFilterWhitoutReload("country", countryObject.countryNameEng);
            }
        }
        if (lang === "Ru") {
            const countryObject = countryObjects.find(item => item.countryNameEng === filterParams.country);
            if (countryObject) {
                handleChangeFilterWhitoutReload("country", countryObject.countryNameRu);
            }
        }
    }, [lang]);
