...
const Dropdown = ({ label, selectedFilter, arrayRu, arrayEn, name, handleChangeFilter, first }: DropdownProps): React.ReactElement => {
        ...
        <div className={styles.wrapper + " " + name}>
            <div className={styles.visibleArea} onClick={() => setIsOpen(prev => !prev)}>
                <div className={styles.selectedFilterCol}>
                    <p>{label}</p>
                    {name === "genre"
                        ? selectedFilter !== "all" && <p>{lang === "Ru" ? translateGenre(selectedFilter) : selectedFilter}</p>
                        : selectedFilter !== "all" && <p>{selectedFilter}</p>
                    }
                </div>
                <div className={styles.arrowCol + (isOpen ? " " + styles.arrowColRev : "")}>
                    <BsChevronDown />
                </div>
            </div>
            <div className={styles.dropdownArea + (isOpen ? ` ${styles.dropdownOpen}` : "") + (first ? ` ${styles.dropdownLeft}` : ` ${styles.dropdownRight}`)}>
                {lang === "Ru" && sortedArrayRu.map(item =>
                    name === "genre"
                        ? <p
                            key={item + " " + Date.now().toString()}
                            className={styles.dropdownItem +
                                (item.toLowerCase() === translateGenre(selectedFilter).toLowerCase()
                                    ? " " + styles.selectedItem
                                    : "")}
                ...
                {lang === "En" && sortedArrayEn.map(item =>
                    name === "genre"
                        ? <p
                            key={item + " " + Date.now().toString()}
                            className={styles.dropdownItem +
