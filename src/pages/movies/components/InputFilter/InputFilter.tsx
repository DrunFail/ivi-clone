...
import { IFilterParams, ISuggestObject, ISuggestResponse } from "../../../../models";
...
const InputFilter = ({ name, label, value, handleChangeFilter, filterParams, getFirstFilmcards, first }: InputFilterProps): React.ReactElement => {
    ...
    async function getAutosuggest(e: React.KeyboardEvent<HTMLInputElement>): Promise<void> {
        if (e.target instanceof HTMLInputElement) {
            if (e.target.value !== "") {
                try {
                    const response = await axios.get(
                        `${domen}persons/search?profession=${name === "director" ? "режиссер" : "актер"}&name=${e.target.value}&size=${suggestionSize}`
                    );
                    ...
                    suggestionsFromDB.length > 0
                        ? setIsOpen(true)
                        : setIsOpen(false);
                    const sortedSuggestionsFromDB = suggestionsFromDB.sort((a, b) => {
    ...
    return (
        <div className={styles.wrapper}>
            <label htmlFor={name + "FilterInput"}>
                {label}
            </label>
            <input
                type="text"
                ...
                id={name + "FilterInput"}
                value={value}
                onChange={e => handleChangeFilter(name, [e.target.value, "-1"])}
            ...
            />
            <div
                className={styles.dropdownArea + (isOpen ? " " + styles.dropdownOpen : "") + (first ? ` ${styles.inputDropdownLeft}` : ` ${styles.inputDropdownRight}`)}
                onClick={handleClick}
            >
                {suggestions && suggestions.map(suggestion =>
                    <div
                        className={styles.dropdownItem +
                            (suggestions.length === 2 ? " " + styles.dropdownItem_twoItem : "") +
                            (suggestions.length === 1 ? " " + styles.dropdownItem_oneItem : "")
                        }
                        key={name + " suggestion " + suggestion.personId.toString()}
