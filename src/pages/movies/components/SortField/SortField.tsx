...
const SortField = ({ sortParam, changeSortParam }: SortFieldProps): React.ReactElement => {
    ...
    useEffect(() => {
        if (!elemInFocus?.closest(`.${styles.visibleArea}`) &&
    ...
    function convertSortParamToLabel(sortParam: string): string {
        const sortParamsObj = sortParams.find(item => sortParam === item.name);
        if (sortParamsObj) {
            return lang === "Ru" ? sortParamsObj.rusLabel : sortParamsObj.engLabel;
        } else {
            return "";
        }
    }
    ...
    function convertSortParamToName(sortParam: string): string {
        const sortParamsObj = sortParams.find(item => sortParam === item.engLabel || sortParam === item.rusLabel);
        if (sortParamsObj) {
            ...
            <div className={styles.dropdownArea + (isOpen ? ` ${styles.dropdownOpen}` : "")}>
                {sortParams.map(item => (
                    <p
                        ...
                        className={styles.dropdownItem + (item.name === sortParam ? " " + styles.selectedItem : "")}
